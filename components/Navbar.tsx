          ...
          <div className="flex justify-between items-center">
            ...
            <a href="#" onClick={(e) => handleNavClick(e, 'home')} className="flex items-center group relative z-50">
              <img 
                src={LOGO} 
                alt={APP_NAME}
                className={`h-12 md:h-14 transition-opacity duration-300 ${isScrolled ? 'brightness-100' : 'brightness-0 invert'}`}
                onError={(e) => {
              ...
              />
              <div className="hidden font-heading text-2xl md:text-3xl font-bold tracking-tighter uppercase transition-colors duration-300" style={{ display: LOGO ? 'none' : 'block' }}>
            ...
            <nav className="hidden lg:flex space-x-8 items-center">
              {NAV_ITEMS.map((item) => (
                <a
                  ...
                  className={`text-sm font-semibold tracking-wide transition-colors duration-300 hover:text-brand-500 uppercase ${isScrolled ? 'text-gray-700' : 'text-white/90 hover:text-white'}`}
                >
                  {item.label}
                </a>
              ))}
              ...
              <a 
                ...
                className={`px-6 py-2.5 text-sm font-bold uppercase tracking-wider rounded-sm transition-all shadow-md transform hover:-translate-y-0.5 ${
                  isScrolled 
            ...
            {/* Mobile Menu Button */}
            <button
              className={`lg:hidden p-2 rounded-md z-50 relative transition-colors ${isScrolled || isOpen ? 'text-gray-800' : 'text-white'}`}
